...
describe('AuthenticationService', () => {
  ...
  const configureTestingModule = (setLocalStorageItem = false): void => {
  ...
  describe('without cached data', () => {
    ...
    it('should get the user by id', fakeAsync(() => {
      const sub = service.getUserByUserId('123').subscribe((user: User) => {
        expect(user).toEqual(mockUser);
      });
      mockHttp
        .expect('POST', '/authentication/user_by_user_id')
        .body({ userId: '123' })
        .send(mockUser);
